...
main($CPU,$MEM,$read_type,$sample_name,$sample_name_short,$mosq_fullname,$date,$location,$filtering,$host_db,$rna_db,$assemble,$assembler,$assemble_output,$viral_contig_search,$virfinder_model,$vircontig_list,$mapping,$orf_prediction,$diamond_db,$hmmer_db,$lineage);


sub main {
    my ($CPU,$MEM,$read_type,$sample_name,$sample_name_short,$mosq_fullname,$date,$location,$filtering,$host_db,$rna_db,$assemble,$assembler,$assemble_output,$viral_contig_search,$virfinder_model,$vircontig_list,$mapping,$orf_prediction,$diamond_db,$hmmer_db,$lineage) = @_;
...
sub chimera_contig {
    ...
    foreach my $node_list (@node_list) {
        ...
        if(-d $node_list){
            ...
            #make GENBANK file
            if($orf_prediction =~ /yes/){
                system("gmhmmp -m ../../../../pipeline/MetaGeneMark_v1.mod -o ProteinPredictionGFF.$sample_name.$node_list.txt -f 3 -a -A PredictedProteins.$sample_name.$node_list.fasta ./$node_list.consensus.second.$sample_name.fasta");
                ...
                system("gmhmmp -m ../../../../pipeline/MetaGeneMark_v1.mod -o ProteinPredictionGFF.$sample_name.$node_list.revcomp.txt -f 3 -a -A PredictedProteins.$sample_name.$node_list.revcomp.fasta ./$node_list.consensus.second.$sample_name.revcomp.fasta");
